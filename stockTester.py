...
# get files to run from user
print('Give tickers of stocks to test. Give an empty line when done.')
tickers = []
while True:
    booty = input('Stock Ticker: ').upper()
    if booty == '':
        break
    else:
        tickers.append(booty)
...
for ticker in tickers:
    complete_file_name = os.path.join(file_path, ticker + '_prices.txt')
    file = open(complete_file_name, "r")
    stock = Stock(ticker)
    # i changed how i did this so the dummy gets rid of first line
    dummy = file.readline()
    ...
    while True:
        ...
        if stock.state == 7:
            ...
            if stock.current_price >= resistance and stock.shares_bought > 0:
                ...
                stock_net = stock_net + net_of_trade
                # reset shares bought
    ...
    print('\nLowest price we could\'ve bought ' + ticker + ' at: ' + str(lowest_price))
    print('Highest price we could\'ve sold ' + ticker + ' at: ' + str(highest_price))
    print(ticker + ' profit: ' + stock_net.__format__('.2f'))
    stock_profit = (100.0 * (account_equity + stock_net) / account_equity).__format__('.4f')
    print(stock_profit + '%\n')

# print results
print('Net profit: ' + net_profit.__format__('.2f'))
